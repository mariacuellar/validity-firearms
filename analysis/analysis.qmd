...
```{r define-flaw-severity-framework}
...
```{r load-packages}
# Preamble
list.of.packages <- c("tidyverse", "ggthemes", "gridExtra", "here")
...
```{r define-figure-output-folder}
figure_output_dir <- here(here(), "results/figures")
...
examiner_panel <- tibble(
  ...
  examiner_skill = rnorm(n_examiners, mean = 0, sd = examiner_sd / 2), # AL: flipped these SDs so there's more heterogeneity in inconclusives than skill
  examiner_inconclusive_tendency = rnorm(n_examiners, mean = 0, sd = examiner_sd )
)
```
...
simulate_response <- function(ground_truth,
  ...
  inconclusive_probability <- plogis(
    qlogis(inconclusive_rate) +
      0.5 * decision_challenge + # AL: why 0.5??
      examiner_inconclusive_tendency
  )
...
save(
  sim_data,
  file = here(here(), "data/sim_data.rda")
)
```
...
- Each examiner evaluates the same set of items. (*AL:* I think we should adjust this so that each examiner answers a max of 30 questions (following Ames II?) or maybe this could be another option to set?)

...
format:
  revealjs:
    theme: [default, icfis-saguaro.scss]
    slide-number: true
    incremental: true
    footer: "ICFIS 2026"
...
# Deck color scheme: Saguaro from NatParksPalettes (n = 4). Keep in sync with
# icfis-saguaro.scss. To go back to viridis, point `theme:` at icfis-viridis.scss
# and restore the figure colors from git history (they used viridis scales).
# The first four are the n = 4 palette; "red" and "green" are Saguaro's extra two.
# The three-category figures use blue / amber / green: the best 3-color subset of
# Saguaro under simulated color-blindness (worst-case delta-E 26.7, vs 11.5 for the
# original blue / orange / olive, whose orange and olive merged for red-green CVD).
saguaro <- as.character(NatParksPalettes::natparks.pals("Saguaro", n = 6))
names(saguaro) <- c("blue", "orange", "olive", "amber", "red", "green")
```
...
| C | Non-representative testing conditions | Damages the data |
...
::: notes
Talk through first row/data collection process first. Then make flaws appear and talk about them very briefly. Then, add the top layer
:::
...
```{=html}
<style>
  table.decision-table { width: 85%; margin: 0 auto 0.9em auto; border-collapse: separate;
                         border-spacing: 6px; text-align: center; font-size: 0.7em; }
  table.decision-table th, table.decision-table td { border: none; padding: 0.35em 0.6em; }
  .reveal table.decision-table thead th { font-weight: bold; border-bottom: none; }
  table.decision-table tbody th { text-align: right; font-weight: bold; white-space: nowrap; }
  table.decision-table td { color: white; font-weight: bold; border-radius: 4px; }
  table.decision-table td.correct      { background: #1e6b3a; } /* dark green  */
  table.decision-table td.incorrect    { background: #8b1a1a; } /* dark red    */
  table.decision-table td.inconclusive { background: #9c7a00; } /* dark yellow */
</style>
<table class="decision-table">
  <thead>
    <tr><th></th><th>Identification</th><th>Inconclusive</th><th>Elimination</th></tr>
  </thead>
  <tbody>
    <tr>
      <th scope="row">Same-source (SS)</th>
      <td class="correct">Correct</td>
      <td class="inconclusive">Inconclusive</td>
      <td class="incorrect">False negative</td>
    </tr>
    <tr>
      <th scope="row">Different-source (DS)</th>
      <td class="incorrect">False positive</td>
      <td class="inconclusive">Inconclusive</td>
      <td class="correct">Correct</td>
    </tr>
  </tbody>
...
::: {.definition}
Target estimand: overall population FPR or FNR or IR is **fixed** for each simulation
:::
...
$P(\text{Error})$ and $P(\text{Inconclusive})$ are both shifted by $c_{ij}$

$$\pi_{ij}^{err} = P(\text{error}) = \text{expit}[\text{logit}(\text{FPR or FNR}) + c_{ij}]$$

$P(\text{Inconclusive})$ is also shifted by $\tau_j$
...
- We used Ames II to benchmark most parameters:  
    - Cartridge cases (raw responses): baseline FPR, FNR, and inconclusive rates, fit by maximum likelihood
        - same-source and different-source items get *separate* inconclusive baselines: about 17% vs. 50%
    - $\sigma_s$, $\sigma_d$, and $\sigma_\tau$ are based on ..... 
    - [todo: How $h$ was set]
...
::: {.notes}
Known misses: (1) firearm-level variation
...
## Calibrating to real studies: gut check 

```{r}
# Pre-computed from the Ames II data by R/prepare_ames2_slide_data.R
ames2_counts <- readr::read_csv(here::here("icfis/data/ames2_firearm_counts.csv"), show_col_types = FALSE)

# Real-study response proportions (missing/unsuitable responses excluded)
ames2_counts |>
  filter(response != "NA") |>
  group_by(m_nm, response) |>
  summarize(n = sum(n), .groups = "drop_last") |>
  mutate(p = n / sum(n)) |>
  select(-n) |>
  pivot_wider(names_from = response, values_from = p, values_fill = 0) |>
  column_to_rownames("m_nm") |>
  round(2) |>
  knitr::kable()
```
...
- For each simulated study, compute both numbers from the same raw
  ...
  - **Worst case**: recode every inconclusive as the error-favoring label,
    recompute
- Compare to the "true" error rate ([ADD EXPLANATION])
...
x_label <- "True rate"
bounds_cols <- setNames(c(saguaro[["blue"]], saguaro[["orange"]]), c("Reported", x_label))
...
# Alternative version with all three layers as facet rows; not shown on the slide.
p_final <- ggplot(points_long, aes(x = setting)) +
  geom_errorbar(
    data = brackets, aes(ymin = reported, ymax = worst_case),
    width = 0.18, linewidth = 0.9, color = saguaro[["blue"]]
  ) +
  ...
  scale_color_manual(values = bounds_cols) +
...
```{r inconclusive-bounds-1-reported}
#| fig-width: 12
#| fig-height: 6
#| classes: fragment
points_long |>
  filter(measure == "Reported") |>
  ggplot(aes(x = setting)) +
  geom_point(aes(y = value, shape = measure, color = measure), size = 3.5, stroke = 1.4) +
  facet_grid( ~ rate) +
  scale_shape_manual(values = setNames(c(16, 4), c("Reported", x_label))) +
  scale_color_manual(values = bounds_cols) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1), limits = c(0,.15)) +
  labs(
    title = "",
    subtitle = "",
    x = "Baseline inconclusive rates",
...
```{r inconclusive-bounds-2-true-rate}
#| fig-width: 12
#| fig-height: 6
#| classes: fragment
points_long |>
  filter(panel == "No bounds") |>
  ggplot(aes(x = setting)) +
  geom_point(aes(y = value, shape = measure, color = measure), size = 3.5, stroke = 1.4) +
  facet_grid( ~ rate) +
  scale_shape_manual(values = setNames(c(16, 4), c("Reported", x_label))) +
...
```{r inconclusive-bounds-3-bounds}
#| fig-width: 12
#| fig-height: 6
#| classes: fragment
points_long |>
  filter(panel == "With bounds") |>
  ggplot(aes(x = setting)) +
  geom_errorbar(
    data = brackets, aes(ymin = reported, ymax = worst_case),
    width = 0.18, linewidth = 0.9, color = saguaro[["blue"]]
  ...
  scale_shape_manual(values = setNames(c(16, 4), c("Reported", x_label))) +
  scale_color_manual(values = bounds_cols) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1), limits = c(0,.55)) +
  labs(
    title = "",
    subtitle = "",
    x = "Baseline inconclusive rates",
...
#| classes: fragment
ames2_counts |>
  mutate(Type = ifelse(ber == "Non_Ber", "Ruger", ber)) |>
  filter(response != "NA") |>
  group_by(Type, m_nm) |>
  summarize(pct_inc = sum(n[response == "Inconclusive"]) / sum(n), .groups = "drop") |>
  pivot_wider(names_from = "m_nm", values_from = "pct_inc") |>
...
#| fig-width: 10
#| fig-height: 5
ames2_long <- ames2_counts |>
  mutate(firearm = fct_reorder(firearm, firearm_order),
         response = na_if(response, "NA"))

# Overall % inconclusive within each facet (pooled over responses). Missing
# responses ("NA") stay in the denominator, matching the bars, which show
# them as their own grey category.
pct_inc <- ames2_long |>
  group_by(ber, m_nm) |>
  summarize(pct_inc = sum(n[response %in% "Inconclusive"]) / sum(n), .groups = "drop")

ames2_long |>
  ggplot(aes(x = firearm, y = n, fill = response)) + 
  ...
  # ) +
  facet_grid(cols = vars(ber), rows = vars(m_nm), scales = "free_x") + 
  scale_fill_manual(
    values = c(Elimination = saguaro[["blue"]], Identification = saguaro[["amber"]], Inconclusive = saguaro[["green"]]),
    na.value = "grey85"
  ) + 
  theme_minimal(base_size = 18) +
  theme(legend.position = "top") + 
  labs(fill = "", x = "individual_firearm", y = "Proportion of responses")
...
- Different firearm makes/models have different baseline difficulties
    - **Simulation:** simulate baseline/mean for 8 types.
...
Casework "population" of firearms: 8 types at different proportions

1. **Convenience sample:** test cases are drawn from two firearm types (21% of casework by weight), 40 firearms each (80 total)
2. **Cluster sample + reweighting:** A hypothetical alternative study covering all
   eight types (10 firearms each, not in population-correct
   proportions) with type recorded as a covariate
3. **Reweighting the convenience sample:** Applying reweighting to the
    convenience sample from (1), which has zero data on the six
   unsampled types.
...
nonrep_colors <- c(
  "Convenience sample (naive)"          = saguaro[["amber"]],
  "Convenience sample (\"reweighted\")" = saguaro[["green"]],
  "Full-coverage study (reweighted)"    = saguaro[["blue"]]
)
...
- No statistical procedure applied to the convenience samples can recover the
...
- Not all six flaws have the same consequences
    - sorting by *remediable vs. non-remediable* clarifies where the "cumulative-science" argument applies
- **Remediable** (maybe A, D, E, and F with assumptions): reanalysis of existing

...
2. **The fix that could work.** A hypothetical better study covering all
   eight types (10 firearms each — still 80 total, not in population-correct
   proportions) with type recorded as a covariate — reweighting by the true
   population distribution afterward — Chin et al.'s "moderator" /
   big-team-science suggestion.
...
final, most precise number). True population rate: **FPR 1.55%,
FNR 2.76%**.

- Convenience sample (naive): FPR **0.19%**, FNR **0.38%** — roughly
  **7–8x too low** on both
- Convenience sample, "reweighted": FPR **0.21%**, FNR **0.42%** — still
  roughly **7x too low** — barely moved
